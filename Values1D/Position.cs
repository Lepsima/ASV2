...
public struct Position : IAutoUnit {
    ...
    public static Position Mile(float x) => new(x * 1609.344f);

    public float ToMillimeter() => x * 1000f;
    public static Position Millimeter(float x) => new(x * 0.001f);

    public float ToCentimeter() => x * 100f;
    public static Position Centimeter(float x) => new(x * 0.01f);

    public float ToMeter() => x * 1f;
    public static Position Meter(float x) => new(x * 1f);

    public float ToKilometer() => x * 0.001f;
    public static Position Kilometer(float x) => new(x * 1000f);

    public float ToMegameter() => x * 1E-06f;
    public static Position Megameter(float x) => new(x * 1000000f);
    ...
    public Position(Velocity a, Time b) : this(a.Position(b)) { }
    public Position(Time b, Velocity a) : this(a.Position(b)) { }

    public Torque Torque(Force v) => new(v.x * x);
    public Force Force(Torque v) => new(v.x / x);

    public Position(Torque a, Force b) : this(a.Position(b)) { }
    public Position(Force b, Torque a) : this(a.Position(b)) { }
